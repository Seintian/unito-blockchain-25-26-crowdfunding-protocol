...
| **Fee-on-Transfer Tokens** | DeFi Pitfall | `Factory` & `Campaign.pledge` | **HARDENED** | Factory and Campaign both utilize the **Balance Delta Pattern** (`balanceAfter - balanceBefore`) to credit exact net delivered amounts for both reward collateral and backer pledges, preventing insolvency. |
| **Residual Reward Dust** | Subadditivity | `CrowdfundingCampaign.sol` | **HARDENED** | Implemented `recoverExcessRewards()`, allowing the creator to sweep true token surplus without endangering remaining backer claims. |
